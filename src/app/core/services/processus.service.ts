...
export class ProcessusService {
  ...
  private BASE_URL = 'http://localhost:8081/engine-rest';
...
completeTask(taskId: string, variables: any = {}): Observable<any> {
  return this.http.post(
    `${this.BASE_URL}/task/${taskId}/complete`,
    {
      variables: this.formatVariables(variables)
    }
  ).pipe(
    catchError((error) =>
      this.handleError('Complétude de tâche', error)
    )
  );
}
...
getCamundaTasks(): Observable<any> {
  return this.http.get(`${this.BASE_URL}/task`)
    .pipe(
      timeout(5000),
      catchError((error) =>
        this.handleError('Chargement des tâches Camunda', error)
      )
    );
}
